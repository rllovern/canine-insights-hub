...
export default function Keywords() {
  ...
  const load = async () => {
    ...
    for (const r of (rows ?? []) as unknown as RankingRow[]) {
      hist.push(r);
    ...
    const { data: sovRows } = await supabase
      ...
      .order("captured_at", { ascending: false })
      .limit(500);
    setSov((sovRows ?? []) as unknown as SoVRow[]);
    setLoading(false);
  };
  ...
  if (!activeProperty) return <div className="text-sm text-muted-foreground">Select a client to see keyword rankings.</div>;
