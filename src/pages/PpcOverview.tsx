...
export function PpcOverviewBody() {
...
function ActionsHeader({ cfg }: { cfg: ReturnType<typeof useClientMetricConfig> }) {
  const order: MetricKey[] = ["leads", "good_leads", "admissions", "medicaid"];
...
function ActionsKpis({
  totals, prev, cfg,
}: {
  totals: any; prev: any; cfg: ReturnType<typeof useClientMetricConfig>;
}) {
  const order: MetricKey[] = ["leads", "good_leads", "admissions", "medicaid"];
  const visible = order.filter((k) => !cfg.isHidden(k));
  const gridCls =
    visible.length === 4 ? "grid grid-cols-1 sm:grid-cols-4 gap-2" :
...
export default function PpcOverview() {
  return (
    <AppShell title="Google PPC Overview">
      <PpcOverviewBody />
    </AppShell>
  );
}
