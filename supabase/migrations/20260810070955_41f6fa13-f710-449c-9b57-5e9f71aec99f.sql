-- 1. Archive sheet_sales (retain history, remove all app access)
ALTER TABLE public.sheet_sales RENAME TO sheet_sales_archived;
REVOKE ALL ON public.sheet_sales_archived FROM anon, authenticated;
DROP FUNCTION IF EXISTS public.get_sheet_sales_by_report_token(text, date, date);

-- 2. Remove sheet machinery
DROP TABLE IF EXISTS public.sheet_sync_config;
ALTER TABLE public.properties DROP COLUMN IF EXISTS google_sheet_tab;

-- 3. Health summary exposes paused pairs + their reason
DROP FUNCTION IF EXISTS public.get_api_health_summary();
CREATE OR REPLACE FUNCTION public.get_api_health_summary()
 RETURNS TABLE(source text, property_id uuid, property_name text, is_connected boolean, last_success_at timestamp with time zone, last_failure_at timestamp with time zone, last_error_message text, last_run_status text, last_run_at timestamp with time zone, pds_status text, is_paused boolean, pause_reason text)
 LANGUAGE plpgsql
...
BEGIN
  ...
  conn AS (SELECT pds.property_id, pds.source, COALESCE(pds.is_connected, false) AS is_connected, pds.status, pds.last_error FROM public.property_data_sources pds),
  last_success AS (
  ...
  SELECT pr.source, pr.property_id, pr.property_name,
    COALESCE(c.is_connected, false),
    ls.last_success_at,
    CASE WHEN lf.last_failure_at > COALESCE(ls.last_success_at, '-infinity'::timestamptz) THEN lf.last_failure_at ELSE NULL END,
    CASE WHEN lf.last_failure_at > COALESCE(ls.last_success_at, '-infinity'::timestamptz) THEN lf.last_error_message ELSE NULL END,
    la.last_run_status,
    la.last_run_at,
    c.status,
    (c.status = 'paused'),
    CASE WHEN c.status = 'paused' THEN c.last_error ELSE NULL END
  FROM pairs pr
