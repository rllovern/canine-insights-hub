ALTER TABLE public.ghl_lead_facts
  ...
  ADD COLUMN IF NOT EXISTS contact_record_updated_at timestamptz,
...
BEGIN
  ...
  IF _issue_type = 'unknown_response_source' THEN
    ...
    SELECT
      ...
      COALESCE(u.name, du.name), (c.assigned_user_id IS NULL AND p.default_lead_owner_user_id IS NOT NULL),
      NULL::timestamptz, NULL::text, NULL::text, NULL::public.ghl_canonical_stage,
      m.sent_at, 'unknown outbound message'::text, NULL::timestamptz,
      NULL::int, NULL::int,
    ...
    WHERE m.response_source = 'unknown' AND m.direction = 'outbound'
  ...
  IF _issue_type = 'appointments_missing_status' THEN
    ...
    SELECT
      ...
      a.starts_at, 'appointment'::text, NULL::timestamptz,
      NULL::int, NULL::int,
      ...
      CASE WHEN pds.config->>'location_id' IS NOT NULL AND a.contact_id IS NOT NULL
        ...
        ELSE NULL END,
      'Appointment status is missing or unknown'::text,
      COALESCE(c.tags, ARRAY[]::text[])
    FROM public.ghl_appointments a
  ...
  IF _issue_type = 'unmapped_stages' THEN
    ...
    SELECT
      ...
      NULL::text,
      'Pipeline stage has not been confirmed in mapping settings'::text,
      ARRAY[]::text[]
    FROM public.ghl_pipeline_stages s
    JOIN public.properties p ON p.id = s.property_id
    LEFT JOIN public.property_pipeline_mapping m
      ON m.property_id = s.property_id AND m.ghl_stage_id = s.ghl_stage_id
    WHERE (_property_ids IS NULL OR s.property_id = ANY(_property_ids))
      AND COALESCE(m.confirmed_by_user, false) = false
    ORDER BY p.name, s.position NULLS LAST, s.name
    LIMIT _limit;
  END IF;
END;
$function$;