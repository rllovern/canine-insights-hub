...
BEGIN
  ...
  SELECT pr.source, pr.property_id, pr.property_name,
    ...
    ls.last_success_at,
    CASE WHEN lf.last_failure_at > COALESCE(ls.last_success_at, '-infinity'::timestamptz) THEN lf.last_failure_at ELSE NULL END AS last_failure_at,
    CASE WHEN lf.last_failure_at > COALESCE(ls.last_success_at, '-infinity'::timestamptz) THEN lf.last_error_message ELSE NULL END AS last_error_message,
    la.last_run_status,
    la.last_run_at
  FROM pairs pr
