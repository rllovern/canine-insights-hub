...
ADD COLUMN IF NOT EXISTS needs_first_response_reason text;
...
CREATE OR REPLACE FUNCTION public.lead_perf_drill(_issue_type text, _property_ids uuid[], _from timestamp with time zone, _to timestamp with time zone, _limit integer DEFAULT 500)
 RETURNS TABLE(property_id uuid, property_name text, contact_id text, contact_name text, phone text, email text, assigned_user_id text, agent_name text, lead_created_at timestamp with time zone, stage_id text, stage_name text, canonical_stage ghl_canonical_stage, last_activity_at timestamp with time zone, first_human_response_at timestamp with time zone, speed_to_lead_seconds integer, human_attempt_count integer, issue_type text, ghl_deep_link text, reason text)
 LANGUAGE plpgsql
