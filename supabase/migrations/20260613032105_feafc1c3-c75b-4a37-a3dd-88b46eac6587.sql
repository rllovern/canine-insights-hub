...
BEGIN
  ...
  with_activity AS (
    ...
    FROM filtered f
    LEFT JOIN LATERAL (
      SELECT ev.ts AS last_at, ev.kind AS last_kind
      FROM (
        SELECT
          x.sent_at AS ts,
          CASE
            WHEN x.message_type = 'TYPE_CALL' AND x.direction = 'inbound' THEN
              CASE WHEN COALESCE(NULLIF(x.meta->'call'->>'duration','')::int, 0) >= 30
                   ...
                   WHEN lower(COALESCE(x.meta->'call'->>'status', x.raw->>'status','')) IN ('no-answer','voicemail','busy','failed','missed')
                   THEN 'missed inbound call'
                   ELSE 'inbound call' END
            WHEN x.message_type = 'TYPE_CALL' AND x.direction = 'outbound' THEN
              CASE WHEN lower(COALESCE(x.meta->'call'->>'status', x.raw->>'status','')) IN ('no-answer','voicemail','busy','failed')
                   THEN 'outbound call, no answer'
                   WHEN COALESCE(NULLIF(x.meta->'call'->>'duration','')::int, 0) >= 30
                   THEN 'outbound call'
                   ELSE 'outbound call attempt' END
            WHEN x.direction = 'inbound' AND x.channel = 'sms' THEN 'customer SMS'
            WHEN x.direction = 'inbound' AND x.channel = 'email' THEN 'customer email'
        ...
        UNION ALL
        SELECT COALESCE(t.completed_at, t.updated_at, t.due_at) AS ts,
          CASE WHEN t.completed_at IS NOT NULL THEN 'task completed' ELSE 'task updated' END AS kind
        FROM public.ghl_tasks t
        WHERE t.property_id = f.property_id AND t.contact_id = f.contact_id
          AND COALESCE(t.completed_at, t.updated_at, t.due_at) IS NOT NULL

        UNION ALL
        SELECT GREATEST(a.updated_at, a.starts_at) AS ts,
        ...
        WHERE a.property_id = f.property_id AND a.contact_id = f.contact_id

        UNION ALL
        SELECT h.changed_at AS ts, 'stage change' AS kind
        FROM public.ghl_opportunity_stage_history h
        JOIN public.ghl_opportunities oo ON oo.id = h.opportunity_id
        WHERE h.property_id = f.property_id AND oo.contact_id = f.contact_id

        UNION ALL
        SELECT o2.ghl_updated_at AS ts, 'opportunity updated' AS kind
        FROM public.ghl_opportunities o2
        WHERE o2.property_id = f.property_id AND o2.contact_id = f.contact_id
          AND o2.ghl_updated_at IS NOT NULL

        UNION ALL
        SELECT cc.updated_at AS ts, 'contact updated' AS kind
        FROM public.ghl_contacts cc
        WHERE cc.property_id = f.property_id AND cc.ghl_contact_id = f.contact_id
          AND cc.updated_at IS NOT NULL AND cc.updated_at > f.lead_created_at
      ) ev
      WHERE ev.ts IS NOT NULL AND ev.ts >= f.lead_created_at
      ORDER BY ev.ts DESC
      LIMIT 1
    ) la ON true
  )
  SELECT
    ...
    COALESCE(
      GREATEST(wa.last_at, wa.last_activity_at),
      wa.last_at, wa.last_activity_at, wa.lead_created_at
    ) AS last_activity_at,
    COALESCE(
      wa.last_kind,
      CASE
        WHEN wa.last_activity_at IS NOT NULL AND (wa.last_at IS NULL OR wa.last_activity_at >= wa.last_at)
          THEN 'activity'
        ELSE 'lead created'
      END
